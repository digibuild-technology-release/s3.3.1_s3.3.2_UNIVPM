...
def split_day_night(df, h_start_work=9, h_stop_work=18):
    df = df.copy()  # Evita di modificare il DataFrame originale
    df['DATE'] = pd.to_datetime(df['DATE'])
    df = df.round(2)
    
    df['hour'] = df['DATE'].dt.hour  # Estrarre direttamente l'ora senza ciclo
    
    # Filtrare direttamente i DataFrame senza usare il ciclo for
    df_work = df[(df['hour'] >= h_start_work) & (df['hour'] <= h_stop_work)]
    df_night = df[~df.index.isin(df_work.index)]  # Seleziona le righe non appartenenti a df_work
...
def split_day_nightvecchia(df, h_start_work, h_stop_work ): #INPUT_CSV_FILE, 
    # df=df_or
    h_start_work=9
    h_stop_work=18
