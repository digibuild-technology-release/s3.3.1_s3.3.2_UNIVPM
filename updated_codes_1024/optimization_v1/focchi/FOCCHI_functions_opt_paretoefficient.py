...
def print_row(file_path, row, mode):
 ...
 #   SPLIT DAY AND NIGHT
 ###################################################   
def split_day_night_focchi(df, h_start_work, h_stop_work ): #INPUT_CSV_FILE, 
    df['date'] = pd.to_datetime(df['date'], format="%d/%m/%Y %H:%M")
    # df=df[['date', 'temp', 'indoor T', 'indoor RH', 'power', 'TSV_now']]
    df=df.round(2)
    ...
    df_work = pd.DataFrame(df_work, columns = ['ID', 'value', 'date', 'indoor T ', 'indoor RH ', 'co2 ', 'outT',
           'tsv', 'hour'])
    df_night = pd.DataFrame(df_night, columns = ['ID', 'value', 'date', 'indoor T ', 'indoor RH ', 'co2 ', 'outT',
           'tsv', 'hour'])
...
def plotParetoSet(best_records, records, lista, K):
    ...
    for idx, t in enumerate(tuples_to_plot):
        ...
        message = (
            f"The indoor temperature and indoor humidity suggested to the end-user are:\n"
