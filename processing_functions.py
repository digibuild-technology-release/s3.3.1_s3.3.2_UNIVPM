...
def dataset_input(dataset):
    ...
    outNext=dataset['outT_avg'].shift(-24)
...
def dataset_inputmod(dataset): 
    # Verifica se la colonna 'datetime' esiste nel dataset
    if 'datetime' not in dataset.columns:
        raise ValueError("La colonna 'datetime' non esiste nel dataset")
    if not pd.api.types.is_datetime64_any_dtype(dataset['datetime']):
      dataset['datetime'] = pd.to_datetime(dataset['datetime'])  # Converte la colonna 'date' in datetime se non lo è già
    dataset.set_index('datetime', inplace=True)  # Imposta 'date' come indice
    #add hour and day of the week 
    dataset['hour'] = dataset.index.hour 
    dataset['day_of_week'] = dataset.index.weekday #weekday mi da il numero intero riferito al giorno, altrimenti con la stringa mi dava errore
    if 'Unnamed: 0' in dataset.columns:
        dataset = dataset.drop(columns=['Unnamed: 0'])
    ####add outdoor for the next day
    dataset["outT_mean"] = dataset["outT_avg"].shift(-24) # Usa 'outT_avg' che è già stato calcolato
    dataset["outNext"] = dataset["outT_mean"].shift(-24)
    print("outNext after adding:", dataset["outNext"].head())  # Stampa i primi 5 valori di outNext per verificarne il contenuto
